...
- [x] CA-08: Script `scripts/setup-render-qa-env.ps1` permite cargar variables runtime en Render QA via API.
...
# Notas de Despliegue
Configurar en GitHub los secretos `EXPO_TOKEN`, `RENDER_API_KEY`, `NEON_DATABASE_URL`, `NEON_DATABASE_USERNAME`, `NEON_DATABASE_PASSWORD`, `APP_JWT_PRIVATE_KEY`, `APP_JWT_PUBLIC_KEY` y la variable `QA_BASE_URL`. Para Render runtime, ejecutar `scripts/setup-render-qa-env.ps1` con `RENDER_API_KEY` local. SonarQube no requiere Render ni billing; si existe una instancia externa, agregar `SONAR_TOKEN`, `SONAR_HOST_URL` y `SONAR_PROJECT_KEY`.
